...
const SummaryPage: React.FC = () => {
  ...
  const formData = {
    ...
    checkboxes: query.checkboxes
      ? JSON.parse(parseQueryParam(query.checkboxes))
    ...
    components: query.components
      ? JSON.parse(parseQueryParam(query.components))
      : [],
  };
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        סוג פרויקט: {formData.projectType}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        שם הפרויקט: {formData.projectName}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        מחיר: {formData.budget} ₪
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                    ...
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        אחוז: {formData.percentage}%
                      </Typography>
                    </Grid>
                    {formData.dates.map(
                      (
                        date: { openDate: string; closeDate: string },
                        index: number
                      ) => (
                        <Grid container spacing={1} key={index}>
                          <Grid item xs={12}>
                            <Typography variant='body1'>
                              תאריך פתיחה: {date.openDate}
                            </Typography>
                          </Grid>
                          <Grid item xs={12}>
                            <Typography variant='body1'>
                              תאריך סגירה: {date.closeDate}
                            </Typography>
                          </Grid>
                        </Grid>
                      )
                    ...
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        שם תצוגה: {formData.displayName}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        מספר קלט: {formData.numberInput}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        מצב טוגל: {formData.isToggleOn ? 'מופעל' : 'כבוי'}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        צ'קבוקס 1:{' '}
                        {formData.checkboxes.box1 ? 'מסומן' : 'לא מסומן'}
                      </Typography>
                      <Typography variant='body1'>
                        צ'קבוקס 2:{' '}
                        {formData.checkboxes.box2 ? 'מסומן' : 'לא מסומן'}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        רדיו: {formData.radioOption}
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      ...
                      {formData.files.map((file, index) => (
                        ...
                        >
                          <Typography variant='body1'>
                            שם הקובץ: {file.fileName}
                          </Typography>
                          <Typography variant='body1'>
                            שם תצוגה: {file.displayName}
                          </Typography>
                        </Box>
                      ))}
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant='body1'>
                        רכיבי איכות: {100 - formData.percentage}%
                      </Typography>
                      <Typography variant='body1'>
                        רכיבי מחיר: {formData.percentage}%
                      </Typography>
                      {formData.components.map((component, index) => (
                        ...
                        >
                          <Typography variant='body1'>
                            תיאור: {component.description}
                          </Typography>
                          <Typography variant='body1'>
                            מחיר: {component.price}%
                          </Typography>
                        </Box>
                      ))}
                    </Grid>
